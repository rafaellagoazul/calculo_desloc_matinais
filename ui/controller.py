...
from core.persistencia_excel import aplicar_correcoes_excel
...
class AppController:
    ...
    def iniciar_processamento(self, gerar_excel: bool = True):
        ...
        self._ui(lambda: self.app.log("▶ Iniciando processamento"))
    ...
    def reprocessar_falhas(self):
        if not self.falhas:
            AlertModal(
                self.app,
                "Info",
                "Não há falhas para reprocessar.",
                "info"
            )
            return

        self.app.cancel_event.clear()
        self._ui(lambda: self.app.log("🔁 Reprocessando apenas falhas..."))
    ...
    def _reprocessar_falhas_thread(self):
        try:
            ...
            self._ui(lambda: self.app.log(
                f"✅ Falhas resolvidas: {resolvidas}"
            ))
    ...
    def _criar_calculador(self, arquivo_excel, salvar_excel: bool = False):
        ...
        if not ors_key:
            raise RuntimeError(
                "Chave ORS não configurada. Verifique as configurações do sistema."
            )
    ...
    def _registrar_falha(self, linha, origem, destino, motivo):
        ...
        self._ui(lambda: self.app.log(
            f"❌ Linha {linha}: {origem} → {destino} | {motivo}"
        ))
    ...
    def _finalizar(self):
        ...
        self._ui(lambda: self.app.log("🏁 Processamento finalizado"))
    ...
    def _perguntar_abrir_mapa(self):
        AlertModal(
            ...
            "Deseja abrir o mapa para conferência?",
            tipo="question",
