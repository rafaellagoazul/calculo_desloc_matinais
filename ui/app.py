...
from ui.stores.config_store import carregar_config
from ui.stores.history_store import inicializar_db as init_history_db
from ui.stores.deslocamentos_store import inicializar_db as init_deslocamentos_db
from ui.theme import aplicar_tema
...
class App(ctk.CTk):
    ...
    def _arquivo_selecionado(self, path: Path):
        ...
        self.log(f"📂 Arquivo selecionado: {path.name}")
    ...
    def _abrir_manutencao(self):
        AlertModal(
            self,
            "Manutenção",
            "Módulo de manutenção ainda não disponível.",
            "info"
        )
    ...
    def _abrir_sobre(self):
        AlertModal(
            ...
            "\n \nCalculador de Deslocamentos - Versão 2.1.3.1"
            "\n \nCopyright © 2025-2026 - Manra Sistemas"
        )

    def _baixar_modelo(self):
        from ui.services.modelo_excel_service import baixar_modelo_excel
        baixar_modelo_excel(self)  # 👈 AGORA SIM
